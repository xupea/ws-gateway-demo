...
export default function WsDebugger() {
  ...
  const [authToken, setAuthToken] = useState(
    "b3f8cf666a18af715a6d2cc4e25a3220c5de420a761be2e23b2d3de0f071bbed1630c255b8bbabcfc4d8fdd396ce5ee5",
  );
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 p-6 font-mono text-sm">
      ...
      <div className="flex items-center gap-2 mb-4 text-xs">
        <span
          className={`inline-block w-2 h-2 rounded-full ${STATUS_STYLE[status]}`}
        />
      ...
      <div className="bg-gray-900 border border-gray-700 rounded h-80 overflow-y-auto p-4 mb-4">
        ...
        {logs.map((log) => (
          <div key={log.id} className="mb-1 leading-relaxed">
            ...
            <pre
              className={`inline whitespace-pre-wrap break-all ${LOG_STYLE[log.direction]}`}
            >
              {log.content}
            </pre>
          </div>
      ...
      <section className="mb-4">
        ...
        <p className="text-xs text-gray-500 mb-3">
          Topic messages are delivered only after an explicit subscribe.
          Reconnect will restore remembered subscriptions.
        </p>
        ...
        {subscriptions.length > 0 && (
          <div className="space-y-1">
            {subscriptions.map((s) => (
              <div
                key={s.id}
                className="flex items-center justify-between bg-gray-800 border border-gray-700 rounded px-3 py-2 text-xs"
              >
                <div>
      ...
      <section>
        <p className="text-gray-500 text-xs mb-2 uppercase tracking-wide">
          Manual protocol message
        </p>
        <p className="text-xs text-gray-500 mb-3">
          Useful for raw client messages like `ping`, `subscribe`, or
          `complete`.
        </p>
        <div className="flex gap-2">
          <textarea
            ...
            placeholder='{"type":"ping"}  (Cmd/Ctrl+Enter to send)'
            onKeyDown={(e) => {
